...
impl McpServer {
    ...
    fn handle_tools_list(&self, id: Option<serde_json::Value>) -> String {
        let mut tools = vec![
            ...
            tool_def(
                "read_note",
                "Read a note by title (fuzzy matched)",
        ...
        if !self.read_only {
            tools.extend(vec![
                ...
                tool_def(
                    "add_to_journal",
                    "Append a line to a journal entry",
                    &[("content", "string", "Text to append")],
                ),
                tool_def(
                    "toggle_task",
                    "Toggle a task checkbox in a page",
                    &[
                        ("title", "string", "Page title"),
                    ...
                    if let Some(fm) = self.parser.parse_frontmatter(&content) {
                        if let Some(ref t) = fm.title {
                            if t.to_lowercase().contains(&title.to_lowercase()) {
                                ...
                                if count == 0 {
                                    return Err(
                                        "Text not found in page. Re-read and try again.".into()
                                    );
                                }
                                if count > 1 {
