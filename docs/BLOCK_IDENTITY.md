...
**Valid as git tree entries.** Block history is tracked as virtual files in git (see [HISTORY.md](HISTORY.md)). Base36 trivially satisfies git requirements (non-empty, no NUL, no `/`).
...
- [HISTORY.md](HISTORY.md) — git-backed history for self-healing and per-block virtual files
